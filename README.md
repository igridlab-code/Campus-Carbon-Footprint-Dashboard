...
Futuristic digital twin modeling showing carbon analytics, holographic GIS campus overlays, and clean renewable energy distribution systems.
...
Enviora System Architecture Banner

```mermaid
graph TD
    UI["User Interface - Web App and Responsive Mobile UI"] --> ReactTypeScript["Frontend - React 19, TypeScript, Tailwind CSS"]
    ReactTypeScript --> ExpressServer["Backend - Node.js and Express REST API"]
    ...
    ExpressServer --> JWT["Security - JWT Authentication and Role-Based Access"]
...
│   └── ai/                             # Redesigned premium assets & screen crops
│       ├── enviora-thumbnail.png
│       ├── hero-banner.png
│       ├── dashboard-thumbnail.png
│       ├── campus-map-thumbnail.png
│       ├── ai-insights-thumbnail.png
│       ├── sustainability-thumbnail.png
│       ├── asset-management-thumbnail.png
│       ├── user-management-thumbnail.png
│       ├── issue-reporting-thumbnail.png
│       ├── virtual-campus-tour-thumbnail.png
│       ├── admin-dashboard-thumbnail.png
│       └── system-architecture-banner.png
...
```bash
git clone https://github.com/igridlab-code/Campus-Carbon-Footprint-Dashboard.git
cd Campus-Carbon-Footprint-Dashboard
```
...
| `SMTP_PORT` | Port number of target SMTP server (usually 587 for TLS, 465 for SSL). | `587` | **Yes** |
| `SMTP_USER` | Email username used to authenticate with SMTP host. | `indraverseig@gmail.com` | **Yes** |
| `SMTP_PASS` | Password or App Password for SMTP authentication. | `your app password` | **Yes** |
| `SMTP_SECURE` | Set to `true` if your SMTP host requires secure SSL/TLS. | `false` | **Yes** |
| `EMAIL_FROM` | The sender address header injected in dispatched alert mail headers. | `IndraVerse <email>` | **Yes** |
...
Built with ❤️ for Indra Ganesan Institutions
